...
app.use(
    cookieSession({
        secret: "a hard days night by the beatles",
...
app.post("/login", (request, response) => {
    if (request.body) {
        database.readUsersByEmail(request.body.email).then(results => {
            if (results) {
                encrypt
                    .checkPassword(request.body.password, results.password)
                    .then(doesMatch => {
                        if (!doesMatch) {
                            response.json({ success: false });
                        } else {
...
app.post("/getName", (request, response) => {
    database
        .getName(request.session.user.userid)
        .then(results => {
...
app.post("/getFavourites", (request, response) => {
    database
        .getFavourites(request.session.user.userid)
        .then(results => {
            response.json({ success: true, results: results });
        })
        .catch(err => {
...
app.post("/checkFavourites", (request, response) => {
    database
        .checkFavourites(request.session.user.userid, request.body.characterId)
        .then(results => {
            if (results.length > 0) {
                response.json({ success: true });
            } else {
...
app.post("/deleteFavourites", (request, response) => {
    database
        .deleteFavourites(request.session.user.userid, request.body.characterId)
        .then(results => {
            response.json({ success: true, results: results });
        })
        .catch(err => {
